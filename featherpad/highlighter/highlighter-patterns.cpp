...
QStringList Highlighter::keywords (const QString &lang)
{
    ...
    else if (lang == "javascript" || lang == "qml")
    {
        ...
        keywordPatterns << "(?<!\\\\)(\\\\{2})*(?<!((#|\\$|@|\"|\'|`)(?!\\\\)))(\\\\(#|\\$|@|\"|\'|`)){0,1}\\K\\b("
                           "after|append|apply|array|auto_execok|auto_import|auto_load|auto_load_index|auto_mkindex|auto_mkindex_old|auto_path|auto_qualify|auto_reset|binary|bgerror|break|case|catch|cd|clock|close|concat|continue|dde|default|dict|else|elseif|encoding|env|eof|error|errorCode|errorInfo|eval|exec|exit|expr|fblocked|fconfigure|fcopy|file|fileevent|flush|for|foreach|format|gets|glob|global|history|if|info|interp|join|lappend|lindex|linsert|list|llength|lmap|load|lrange|lrepeat|lreplace|lreverse|lsearch|lset|lsort|my|namespace|next|nextto|open|package|pid|pkg_mkIndex|prefix|proc|puts|pwd|read|regexp|regsub|rename|resource|return|scan|seek|set|socket|source|split|string|subst|switch|tcl_library|tcl_patchLevel|tcl_pkgPath|tcl_platform|tcl_precision|tcl_rcFileName|tcl_rcRsrcName|tcl_traceCompile|tcl_traceExec|tcl_version|tclLog|tell|timerate|throw|time|trace|trap|try|unknown|unload|unset|update|uplevel|upvar|variable|vwait|while|yield|yieldto"
                           "|"
                           "bell|bind|bindtags|bitmap|busy|button|canvas|checkbutton|clipboard|destroy|entry|event|focus|font|fontchooser|frame|grab|grid|image|label|listbox|loadTk|lower|menu|menubutton|message|option|options|pack|photo|place|radiobutton|raise|scale|scrollbar|selection|send|text|tcl_endOfWord|tcl_findLibrary|tcl_startOfNextWord|tcl_startOfPreviousWord|tcl_wordBreakAfter|tcl_wordBreakBefore|tk|tk_bisque|tk_chooseColor|tk_chooseDirectory|tk_dialog|tk_focusFollowsMouse|tk_focusNext|tk_focusPrev|tk_getOpenFile|tk_getSaveFile|tk_menuSetFocus|tk_messageBox|tk_optionMenu|tk_popup|tk_setPalette|tk_strictMotif|tk_textCopy|tk_textCut|tk_textPaste|tk_version|tkerror|tkvars|tkwait|toplevel|winfo|wm"
                           "|"
                           "beep|button|chan|checkbutton|combobox|console|entry|frame|incr|label|labelframe|lassign|menubutton|notebook|panedwindow|progressbar|radiobutton|registry|scale|scrollbar|separator|sizegrip|spinbox|style|treeview"
                           ")(?!(@|#|\\$|\"|\'|`))\\b";
