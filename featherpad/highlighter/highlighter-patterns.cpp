...
QStringList Highlighter::keywords (const QString &lang)
{
    ...
    else if (lang == "javascript" || lang == "qml")
    {
        ...
        keywordPatterns << "(?<!\\\\)(\\\\{2})*(?<!((#|\\$|@|\"|\'|`)(?!\\\\)))(\\\\(#|\\$|@|\"|\'|`)){0,1}\\K\\b("
                           "after|append|array|auto_execok|auto_import|auto_load|auto_load_index|auto_mkindex|auto_mkindex_old|auto_qualify|auto_reset|binary|bgerror|break|case|catch|cd|clock|close|concat|continue|dde|default|dict|else|elseif|encoding|env|eof|error|errorCode|errorInfo|eval|exec|exit|expr|fblocked|fconfigure|fcopy|file|fileevent|flush|for|foreach|format|gets|glob|global|history|if|info|interp|join|lappend|lindex|linsert|list|llength|load|lrange|lrepeat|lreplace|lreverse|lsearch|lset|lsort|namespace|open|package|pid|pkg_mkIndex|proc|puts|pwd|read|regexp|regsub|rename|resource|return|scan|seek|set|socket|source|split|string|subst|switch|tcl_library|tcl_patchLevel|tcl_pkgPath|tcl_platform|tcl_precision|tcl_rcFileName|tcl_rcRsrcName|tcl_traceCompile|tcl_traceExec|tcl_version|tclLog|tell|time|trace|unknown|unload|unset|update|uplevel|upvar|variable|vwait|while"
                           "|"
                           "bell|bind|bindtags|bitmap|button|canvas|checkbutton|clipboard|destroy|entry|event|focus|font|frame|grab|grid|image|label|listbox|loadTk|lower|menu|menubutton|message|option|options|pack|photo|place|radiobutton|raise|scale|scrollbar|selection|send|text|tcl_endOfWord|tcl_findLibrary|tcl_startOfNextWord|tcl_startOfPreviousWord|tcl_wordBreakAfter|tcl_wordBreakBefore|tk|tk_bisque|tk_chooseColor|tk_dialog|tk_focusFollowsMouse|tk_focusNext|tk_focusPrev|tk_getOpenFile|tk_getSaveFile|tk_messageBox|tk_optionMenu|tk_popup|tk_setPalette|tkerror|tkvars|tkwait|toplevel|winfo|wm"
                           "|"
