                          ...
                          const QHash<QString, QColor> &syntaxColors) : QSyntaxHighlighter (parent)
{
    ...
    if (progLan == "sh" || progLan == "makefile" || progLan == "cmake")
    {
        ...
        rule.pattern.setPattern ("((^\\s*|[\\(\\);&`\\|{}!=^]+\\s*|(?<=~|\\.)+\\s+)((if|then|elif|elseif|else|fi|while|do|done|esac)\\s+)*)\\K(?<!\\${)(sudo\\s+)?((kill|killall|torify|proxychains)\\s+)?(aclocal|aconnect|adduser|addgroup|aplay|apm|apmsleep|apropos|apt|apt-get|ar|as|aspell|autoconf|autoheader|automake|awk|basename|bash|bc|bison|bsdtar|bunzip2|bzip2|cal|cat|cc|cd|cfdisk|chfn|chgrp|chmod|chown|chroot|chkconfig|chsh|cksum|clang|clear|cmake|cmp|co|col|comm|cp|cpio|cpp|cron|crontab|csplit|curl|cut|cvs|date|dc|dd|ddrescue|df|diff|diff3|dig|dir|dircolors|dirname|dirs|dmesg|dpkg|du|egrep|eject|emacs|env|ethtool|expect|expand|expr|fbset|fdformat|fdisk|featherpad|fgrep|file|find|finger|fmt|fold|format|fpad|free|fsck|ftp|function|fuser|gawk|gc|gcc|gdb|getent|getopt|gettext|gettextize|gio|git|gmake|gpg|grep|groups|gs|gunzip|gzip|head|hexdump|hostname|id|ifconfig|ifdown|ifup|import|install|java|javac|jobs|join|kdialog|kill|killall|ld|ldd|less|links|ln|locate|lockfile|logname|look|lp|lpc|lpr|lprint|lprintd|lprintq|lprm|ls|lsof|lynx|mail|make|makepkg|man|mesg|mkdir|mkfifo|mkisofs|mknod|mktemp|more|mount|mtools|mv|mmv|nano|nasm|nawk|netstat|nice|nl|nm|nmap|nohup|nop|nslookup|od|open|op|pacman|passwd|paste|patch|pathchk|pcregrep|pcretest|perl|perror|pico|pidof|pine|ping|pkill|popd|pr|printcap|printenv|procmail|proxychains|prune|ps|psbook|psmerge|psnup|psresize|psselect|pstops|pstree|pwd|python|qarma|qmake((-qt)?[3-9])*|quota|quotacheck|quotactl|ram|rcp|rcs|readarray|reboot|rename|renice|remsync|rev|rm|rmdir|rsync|ruby|screen|scp|sdiff|sed|seq|setterm|size|sftp|shutdown|skill|sleep|slocate|slogin|snice|sort|sox|split|ssh|stat|strace|strings|strip|stty|su|sudo|suidperl|sum|svn|symlink|sync|tac|tail|tar|tee|tempfile|time|touch|top|torify|traceroute|tr|truncate|tsort|tty|type|ulimit|umask|umount|uname|unexpand|uniq|units|unshar|unzip|updatedb|updmap|uptime|useradd|usermod|users|usleep|uuencode|uudecode|valgrind|vdir|vi|vim|vmstat|w|wall|watch|wc|whatis|whereis|which|who|whoami|wget|write|xargs|xeyes|yad|yes|zenity|zip|zsoelim|zypper|7z)(?!(\\.|-|@|#|\\$))\\b");
        rule.format = keywordFormat;
        highlightingRules.append (rule);
    }
    else
