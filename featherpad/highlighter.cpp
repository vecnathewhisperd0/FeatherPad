                          ...
                          bool showWhiteSpace, bool showEndings) : QSyntaxHighlighter (parent)
{
    ...
    if (progLan == "sh" || progLan == "makefile" || progLan == "cmake")
    {
        if (progLan == "cmake")
        {
            ...
            keywordFormat.setForeground (DarkMagenta);
            rule.pattern.setPattern ("(?<=^|\\(|\\s)(VERSION|ON|OFF|FALSE|TRUE|FILES|TARGETS|RUNTIME|DESTINATION|DEPENDS|ALL|COMMENT|VERBATIM|DIRECTORY|EXACT|QUIET|MODULE|NO_MODULE|REQUIRED|COMPONENT|COMPONENTS|NO_POLICY_SCOPE|CONFIG|CONFIGS|HINTS|PATHS|PATH_SUFFIXES|NO_DEFAULT_PATH|NO_CMAKE_ENVIRONMENT_PATH|NO_CMAKE_PATH|NO_SYSTEM_ENVIRONMENT_PATH|NO_CMAKE_PACKAGE_REGISTRY|NO_CMAKE_BUILDS_PATH|NO_CMAKE_SYSTEM_PATH|NO_CMAKE_SYSTEM_PACKAGE_REGISTRY|CMAKE_FIND_ROOT_PATH_BOTH|ONLY_CMAKE_FIND_ROOT_PATH|NO_CMAKE_FIND_ROOT_PATH|INSTALL_DESTINATION|PATH_VARS|NO_SET_AND_CHECK_MACRO|NO_CHECK_REQUIRED_COMPONENTS_MACRO|VARIABLE_PREFIX|SOVERSION|VERSION_HEADER|PACKAGE_VERSION_FILE|WORKING_DIRECTORY|STATUS|WARNING|AUTHOR_WARNING|SEND_ERROR|FATAL_ERROR|DEPRECATION)(?=$|\\)|\\s)");
            rule.format = keywordFormat;
            highlightingRules.append (rule);
        }
