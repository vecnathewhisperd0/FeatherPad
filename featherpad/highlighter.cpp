                          ...
                          bool showWhiteSpace, bool showEndings) : QSyntaxHighlighter (parent)
{
    ...
    if (progLan == "sh" || progLan == "makefile" || progLan == "cmake")
    {
        if (progLan == "cmake")
        {
            ...
            rule.pattern.setPattern ("^\\s*find_package\\s*\\(.*\\K(EXACT|QUIET|MODULE|REQUIRED)(\\s*(?=\\))|\\s+COMPONENTS(?=\\s|\\)))");
            rule.format = keywordFormat;
            highlightingRules.append (rule);

            rule.pattern.setPattern ("^\\s*(find_package|include)\\s*\\(.*\\KNO_POLICY_SCOPE\\s*(?=\\))");
            rule.format = keywordFormat;
            highlightingRules.append (rule);

            rule.pattern.setPattern ("^\\s*(option|set)\\s*\\(.*\\K(ON|OFF|FALSE|TRUE)\\s*(?=\\))");
            rule.format = keywordFormat;
            highlightingRules.append (rule);
        }
